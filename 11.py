...
# NOTE: it isn't clear if "wrapping" solutions are valid or not, but it
# turns out not to matter. My solution produces wrapping answers.

# NOTE: python doesn't like the formatting of the original numbers;
# like C a starting "0" denotes octal instead of decimal. nuke starting
# zeroes.
G = [
...
# or diagonally) can be defined as specializations of the same pattern, that
# of iteration by a given step.
...
max = pattern("vert", G, 20, 4, max)
max = pattern("dia\\",G, 21, 4, max)
max = pattern("dia/", G, 19, 4, max)
